...
- 作成パラメータ履歴のLocalStorage保存（最大10件）
...
12. 作成したデータ印のパラメータ履歴をLocalStorageに最大10件保存できること。
13. 履歴一覧から任意の履歴を選択すると、その内容で編集を再開できること。
14. すべての処理がブラウザ内で完結し、外部通信を行わないこと。
...
### 8.5 ローカルデータ保持
- 履歴データはブラウザのLocalStorageに保存され、同一ブラウザ環境で再利用できること。
- 履歴件数は最大10件に制限し、保存容量の増大を抑制すること。

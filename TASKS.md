...
- notes:
  - `src/history/storage.ts` に履歴保存/読込（最大10件）を実装し、破損データ時のフォールバックを追加。
  - `src/history/historyList.ts` で履歴一覧描画と選択コールバックを実装。
  - `src/main.ts` に履歴保存ボタンと復元処理を接続し、選択時にフォーム/プレビューを再現できるよう更新。

...
- `npm run test -- storage`
  - 期待結果: `storage` モジュールのユニットテストが成功する。
  - 実績: 成功。履歴未保存時の空配列、11件目保存時の古い履歴削除、破損JSON時のフォールバックを検証。
- `npm run build`
  - 期待結果: 履歴機能を追加後も本番ビルドが成功する。
  - 実績: 成功。`dist/` 生成を確認。
- `npm run lint`
  - 期待結果: 追加した履歴モジュールとUI連携コードでLintエラーがない。
  - 実績: 成功。エラー0件。

### 回帰防止テストケース（手動）
1. 初期表示時、履歴が未保存であれば「履歴はまだありません。設定を保存するとここに表示されます。」が表示される。
2. 「現在の設定を履歴に保存」ボタン押下で履歴一覧に1件追加される。
3. 11件以上保存すると、表示/保存される履歴が最新10件に保たれ、最古の1件が削除される。
4. 履歴項目をクリックすると、上枠・日付・下枠・書体・文字サイズ倍率・色・枠線設定がフォームへ復元され、Canvasプレビューも一致して更新される。
5. ブラウザを再読み込みしても、同一ブラウザ環境ではLocalStorageから履歴一覧が再表示される。

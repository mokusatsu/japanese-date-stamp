...
### 実行済みチェック
- `npm run test`
  - 期待結果: 既存ユニットテストが全件成功し、統合変更で回帰がない。
  - 実績: 成功。全テストが通過。
- `npm run lint`
  - 期待結果: 変更後コードのLintエラーがない。
  - 実績: 成功。エラー0件。
- `npm run build`
  - 期待結果: 本番ビルドが成功し、配布可能な成果物を生成できる。
  - 実績: 成功。`dist/` 生成を確認。
- `rg -n "fetch\\(|XMLHttpRequest|WebSocket|EventSource|sendBeacon|navigator\\.sendBeacon|axios" src`
  - 期待結果: 外部通信APIを利用する実装が存在しない。
  - 実績: 成功。該当なし。
- `rg -n "reference-layout\.svg" src/main.ts`
  - 期待結果: 参照SVGのURLが `import.meta.env.BASE_URL` ベースで解決され、サブパス配信でも表示できる。
  - 実績: 成功。`referenceLayoutUrl` を経由した参照を確認。

### 回帰防止テストケース（手動）
1. `docs/manual-test.md` に沿って入力→出力→履歴までのE2E手順を実施し、主要フローが完了できる。
2. ブラウザ開発者ツールのNetworkタブで、操作時に外部ドメインへの通信が発生しない。
3. サブパス配信（例: GitHub Pages）でも参考レイアウトSVGが表示される。
4. READMEの手順どおりに `npm ci` / `npm run dev` / `npm run build` を実行し、セットアップ〜ビルドまで完了する。

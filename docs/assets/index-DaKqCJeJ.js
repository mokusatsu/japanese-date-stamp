...
<svg xmlns="http://www.w3.org/2000/svg" width="${t.size}" height="${t.size}" viewBox="0 0 ${t.size} ${t.size}">
  ...
  <text x="${e.centerX}" y="${e.topTextY}" fill="${t.textColor}" text-anchor="middle" dominant-baseline="middle" font-size="${Math.round(S)}" font-family='${l}'>${w(t.topText)}</text>
  <text x="${e.centerX}" y="${e.middleTextY}" fill="${t.textColor}" text-anchor="middle" dominant-baseline="middle" font-size="${Math.round(v)}" font-family='${l}'>${w(t.dateText)}</text>
  <text x="${e.centerX}" y="${e.bottomTextY}" fill="${t.textColor}" text-anchor="middle" dominant-baseline="middle" font-size="${Math.round(T)}" font-family='${l}'>${w(t.bottomText)}</text>
</svg>`},ft=t=>{const e=pt(t),a=new Blob([e],{type:"image/svg+xml;charset=utf-8"}),o=URL.createObjectURL(a);try{const r=t.fileName??"japanese-date-stamp.svg";(t.triggerDownload??mt)(o,r)}finally{URL.revokeObjectURL(o)}},ht=t=>`${L(t.values.date,t.values.eraFormat,t.values.dateSeparator)} ${t.values.topText} / ${t.values.bottomText}`,gt=({listElement:t,entries:e,onSelect:a})=>{if(t.innerHTML="",e.length===0){const o=document.createElement("li");o.className="rounded-md border border-dashed border-slate-300 px-3 py-2 text-slate-500",o.textContent="履歴はまだありません",t.append(o);return}for(const o of e){const r=document.createElement("li"),l=document.createElement("button");l.type="button",l.className="w-full rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-100",l.textContent=ht(o),l.addEventListener("click",()=>a(o)),r.append(l),t.append(r)}},B="japanese-date-stamp.history.v1",P=10,X=(t=localStorage)=>{const e=t.getItem(B);if(!e)return[];try{const a=JSON.parse(e);return Array.isArray(a)?a.slice(0,P):[]}catch{return[]}},xt=(t,e=localStorage)=>{const a=X(e),r=[{id:crypto.randomUUID(),createdAt:new Date().toISOString(),values:t},...a].slice(0,P);return e.setItem(B,JSON.stringify(r)),r},A=document.querySelector("#app"),bt="./reference-layout.svg";if(!A)throw new Error("App root not found");A.innerHTML=`
      ...
      <div class="space-y-6">
        <section class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          ...
          <div class="grid place-items-center rounded-lg border border-slate-200 bg-slate-50 p-4">
            <img src="${bt}" alt="データ印レイアウト参考図" class="h-auto w-full max-w-xs" />
...
`;const d=document.querySelector("#topText"),u=document.querySelector("#date"),m=document.querySelector("#bottomText"),p=document.querySelector("#eraFormat"),f=document.querySelector("#dateSeparator"),h=document.querySelector("#fontFamily"),g=document.querySelector("#textScale"),x=document.querySelector("#textColor"),b=document.querySelector("#strokeColor"),y=document.querySelector("#strokeWidth"),k=document.querySelector("#transparentBg"),M=document.querySelector("#downloadPng"),_=document.querySelector("#downloadSvg"),D=document.querySelector("#saveHistory"),W=document.querySelector("#historyList"),c=document.querySelector("#stampCanvas");if(!d||!u||!m||!p||!f||!h||!g||!x||!b||!y||!k||!M||!_||!D||!W||!c)throw new Error("Rendering controls not found");const R=c.getContext("2d");if(!R)throw new Error("Canvas 2D context not found");const $=()=>{const t=L(u.value,p.value,f.value);tt(R,c,{topText:d.value,dateText:t,bottomText:m.value,fontFamily:h.value,textScale:Number(g.value),textColor:x.value,strokeColor:b.value,strokeWidth:Number(y.value)})},yt=()=>({topText:d.value,date:u.value,bottomText:m.value,eraFormat:p.value,dateSeparator:f.value,fontFamily:h.value,textScale:Number(g.value),textColor:x.value,strokeColor:b.value,strokeWidth:Number(y.value),transparentBackground:k.checked}),vt=t=>{d.value=t.topText,u.value=t.date,m.value=t.bottomText,p.value=t.eraFormat,f.value=t.dateSeparator,h.value=t.fontFamily,g.value=String(t.textScale),x.value=t.textColor,b.value=t.strokeColor,y.value=String(t.strokeWidth),k.checked=t.transparentBackground,$()},q=()=>{gt({listElement:W,entries:X(),onSelect:t=>vt(t.values)})},St=[d,u,m,p,f,h,g,x,b,y];for(const t of St)t.addEventListener("input",$),t.addEventListener("change",$);M.addEventListener("click",()=>{st(c,{transparentBackground:k.checked})});_.addEventListener("click",()=>{ft({size:Math.min(c.width,c.height),topText:d.value,dateText:L(u.value,p.value,f.value),bottomText:m.value,strokeColor:b.value,strokeWidth:Number(y.value),textColor:x.value,fontFamily:h.value,textScale:Number(g.value)})});D.addEventListener("click",()=>{xt(yt()),q()});$();q();
